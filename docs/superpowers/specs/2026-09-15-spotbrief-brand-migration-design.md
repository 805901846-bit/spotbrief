...
将产品公开名称从 PatchBrief 更新为 SpotBrief，并把版本升级到 `0.2.0`。此次迁移覆盖用户可见界面、安装入口、项目元数据和对外文档，同时保留旧版内部标识以兼容已经保存的设置及旧书签实例。
...
- 如果旧 PatchBrief 实例已经在当前页面运行，点击新 SpotBrief 书签会通过兼容控制对象聚焦现有实例，不重复注入。
...
- 文件路径中包含 `PatchBrief` 的当前指南、面试材料和架构产物重命名为 `SpotBrief`。
- Git 历史提交信息不改写。
- 若文档必须解释迁移，可写“SpotBrief（原 PatchBrief）”，只在迁移说明中出现一次。

## 测试与验收

- 单元测试验证 `VERSION === '0.2.0'`。
- 安装页 E2E 验证标题、页头和可拖动按钮均显示 SpotBrief，页面不再出现面向用户的 PatchBrief 品牌文案。
...
- README、指南、架构图和面试材料中不得残留作为产品名称使用的 PatchBrief；允许源码兼容标识和迁移说明出现旧名称。
