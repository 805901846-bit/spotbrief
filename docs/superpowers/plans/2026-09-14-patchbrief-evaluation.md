# PatchBrief Evaluation Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Build and run a reproducible paired A/B benchmark that compares natural-language prompts with PatchBrief briefs for real Codex frontend edits.
...
- `src/prompts.mjs`: leakage-safe baseline and PatchBrief prompt construction.
...
- `tasks/<id>/task.json`: public prompt, PatchBrief brief, weights, scope, and grader path.
...
- [ ] Implement `validateTask(task, source)` to require `id`, `title`, `audience` in `novice|developer|product-design`, non-empty `naturalPrompt`, PatchBrief headings `# Frontend Change Brief`, `## 修改目标`, `## 选中目标`, `## 修改约束`, `## 预期结果`, six integer weights totaling 100, relative starter/grader paths, allowed paths, and positive timeout. Implement `loadTasks(root)` to discover sorted task directories and reject duplicates.
...
- [ ] Implement `buildPrompt()` with a common preamble stating that only files in the current workspace may be changed, no hidden tests are visible, the agent must inspect existing code, implement the request, and run public checks. Append only `task.naturalPrompt` for baseline and `task.brief` for PatchBrief.
...
- [ ] Commit with `git commit -m "test: add PatchBrief benchmark tasks"`.
...
### Task 7: CLI orchestration, caching, and PatchBrief contract
...
- [ ] Add PatchBrief contract tests using `PATCHBRIEF_ROOT` with the known local path as default. Verify runtime and README exist, README states the product does not modify source, and all task briefs omit configured sensitive canaries.
...
- [ ] Commit with `git commit -m "docs: explain PatchBrief evaluation workflow"`.
...
- [ ] Run `npm run eval:smoke`; expect four real isolated edits: N01 baseline/PatchBrief and D01 baseline/PatchBrief.
