...
**Goal:** Rename the public PatchBrief product to SpotBrief 0.2.0 while preserving the historical internal namespace and saved browser settings.
...
Do not change `PatchBriefController`, `__PATCHBRIEF__`, `__PATCHBRIEF_AI_BRIDGE__`, storage keys, data attributes, DOM ids, CSS classes, or message names.
...
Import and render `BRAND_NAME` in the panel. Change user-visible AI bridge errors from PatchBrief to SpotBrief. Change screenshot and Markdown download names to:
...
Replace public PatchBrief text in `src/install-page/index.html`, `src/install-page/install.ts`, and `public/preview.svg` with SpotBrief. Use `0.2.0` in visible version labels and payload cache query parameters. Keep AI bridge message types and storage keys unchanged.
...
**Files:**
- Rename: `docs/guides/PatchBrief-零基础使用说明书.md` → `docs/guides/SpotBrief-零基础使用说明书.md`
- Rename: `docs/interview/PatchBrief-AI产品经理-VibeCoding面试辅导.md` → `docs/interview/SpotBrief-AI产品经理-VibeCoding面试辅导.md`
- Rename: `docs/architecture/patchbrief-architecture.html` → `docs/architecture/spotbrief-architecture.html`
- Rename: `docs/architecture/patchbrief.architecture.json` → `docs/architecture/spotbrief.architecture.json`
- Modify: `README.md`
...
Update current documentation to SpotBrief 0.2.0. In the migration design only, retain the phrase `SpotBrief（原 PatchBrief）`. Preserve literal compatibility identifiers such as ``window.__PATCHBRIEF__`` and ``patchbrief:preferences`` and label them as historical internal APIs.
...
```bash
rg -n 'PatchBrief' README.md CONTRIBUTING.md SECURITY.md NOTICE LICENSE docs
```

Expected: matches exist only in the migration specification, historical compatibility identifiers, and code snippets that intentionally document the compatibility boundary; no page title, product description, guide title, interview answer, or architecture title uses PatchBrief as the current brand.
