...
(() => {
  ...
  const form = createFormState(preferences.defaultConstraints);
  ...
  function renderSettings() {
    ...
    panel.innerHTML = `<header class="top"><span class="brand">SpotBrief</span><span class="status">任务设置</span><button class="icon" data-action="close-settings">×</button></header><main class="body">${preview ? `${aiMessage ? `<div class="ai-message">${escapeHtml(aiMessage)}</div>` : ''}<div class="preview">${escapeHtml(preview)}</div>` : `<section class="screenshot-tool"><div class="screenshot-heading"><b>页面截图</b><button class="secondary" data-action="capture">截取页面区域</button></div><div class="screenshot-slot"></div></section><label>全局修改目标</label><textarea name="request">${escapeHtml(form.request)}</textarea><details class="form-section" data-section="code"${form.codeOpen ? ' open' : ''}><summary>相关代码</summary><div class="section-content"><select name="language">${['TSX','JSX','TypeScript','JavaScript','Vue','HTML','CSS','SCSS','Other'].map((x) => `<option${x === form.language ? ' selected' : ''}>${x}</option>`).join('')}</select><textarea class="code" name="code">${escapeHtml(form.code)}</textarea></div></details><details class="form-section" data-section="constraints"${form.constraintsOpen ? ' open' : ''}><summary>修改约束</summary><div class="section-content checks">${constraints.map((x) => `<label><input type="checkbox" name="constraint" value="${x}"${form.constraints.includes(x) ? ' checked' : ''}> ${x}</label>`).join('')}</div></details><label class="ai-option"><input type="checkbox" name="ai-optimize"${form.aiOptimize ? ' checked' : ''}> 使用 AI 优化任务书</label>`}</main><footer class="actions">${preview ? '<button class="secondary" data-action="back">返回</button><button class="secondary" data-action="json">复制 JSON</button><button class="secondary" data-action="download">下载 MD</button><button class="primary" data-action="copy">复制任务书</button>' : '<button class="primary" data-action="generate">生成任务书</button>'}</footer>`;
